...
const lists = ["New & Featured", "Men", "Women", "Kids", "Jordan", "Sale"];

export default function MobileSideBar() {
  const { openMobileNav, setOpenMobileNav, setOverlay } = stateFunc();
  ...
  return (
    <section
      className={`fixed top-0 right-0 w-[90vw] min-h-screen z-[600] bg-snow p-10 ${
        openMobileNav ? "open-side-bar" : "close-side-bar"
      }`}
    >
      <div className="w-8 h-8 absolute top-4 right-4" onClick={closeMobileNav}>
        <svg
          width="18"
          height="18"
          viewBox="0 0 18 18"
        ...
        <ul className=" flex flex-col space-y-5">
          {lists.map((list) => (
            <li key={list}>
              <Link
                href={`/${list}`}
                className="flex items-center space-x-3 text-black text-2xl"
              >
                {list}
              </Link>
            </li>
